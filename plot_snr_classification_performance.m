...
for methind = 1:length(methodnames)
    ...
    for s = 1:length(SNRs)
        SNR = SNRs(s);
        data_file = fullfile(stem_dir,sprintf('/results_opm_sim_space_%d_axis_%d/',space,axis),...
    ...
    for s = 1:length(SNRs)
        SNR = SNRs(s);
        data_dir = fullfile(stem_dir,sprintf('/results_opm_sim_space_%d_axis_%d/',space,axis),...
