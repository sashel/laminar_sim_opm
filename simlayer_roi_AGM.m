function simlayer_roi_AGM(SNR, varargin)
% SIMLAYER_ROI_AGM  Run Alternative Generative Model (AGM) simulations with ROI-based analysis
%
% Use as
%   simlayer_roi_AGM(-20), where the argument is the SNR (db).
%
...
combine_surfaces(white_AGM_mesh, pial_AGM_mesh,fullfile(params.surf_dir,'/sub-01/','surf','ds11_white-ds11_pial.surf.gii'))
pial_white_AGM_mesh = fullfile(params.surf_dir,'/sub-01/','surf','ds11_white-ds11_pial.surf.gii');
...
for meshind = 1:length(simmeshes)
    spm_eeg_smoothmesh_mm(simmeshes{meshind},params.sim_patch_size);
end

white = gifti(white_AGM_mesh);
...
matlabbatch = [];
matlabbatch{1}.spm.meeg.source.sensorshift.D = {greyregfile};
...
% coregister simulated dataset to combined AGM pial/white mesh
matlabbatch = [];
...
matlabbatch{1}.spm.meeg.source.headmodel.meshing.meshes.custom.cortex = {pial_white_AGM_mesh};
