...
for s = 1:length(SNRs)
    ...
    for methind = 1:length(methodnames)
        ...
        for i = 1:length(offsets)
            data_file = fullfile(stem_dir,sprintf('/results_opm_sim_space_%d_axis_%d_ds_spmdev%s/',space,axis,offsets{i}),...
        ...
        for i = 1:length(offsets)
            SNR = SNRs(s);
            data_dir = fullfile(stem_dir,sprintf('/results_opm_sim_space_%d_axis_%d_ds_spmdev%s/',space,axis,offsets{i}),...
