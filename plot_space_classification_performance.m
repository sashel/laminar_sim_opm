...
for s = 1:length(SNRs) % loop across SNRs
    ...
    for methind = 1:length(methodnames) % loop across spurce reconstruction approaches
        ...
        for i = 1:length(space)
            ...
            data_file = fullfile(stem_dir,sprintf('/results_opm_sim_space_%d_axis_%d_ds_spmdev/',space(i),axis),...
        ...
        for i = 1:length(space)
            SNR = SNRs(s);
            data_dir = fullfile(stem_dir,sprintf('/results_opm_sim_space_%d_axis_%d_ds_spmdev/',space(i),axis),...
